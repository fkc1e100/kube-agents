...
- `dws-queue-timeout`: Flag ComputeClasses where flex-start maxWaitingDuration exceeds 86400s / 24h.
- `kueue-cohort-starvation`: Flag Kueue ClusterQueues lacking borrowing limits in shared cohorts.
- `nccl-interconnect-drops`: Flag multi-node GPU workloads lacking nccl-fast-socket DaemonSet or GPUDirect tuning.
