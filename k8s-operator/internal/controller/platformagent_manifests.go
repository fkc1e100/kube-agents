...
func buildCredentialProxyEnv(agent *agentv1alpha1.PlatformAgent) []corev1.EnvVar {
	...
	if harness := agent.Spec.Harness; harness != nil && harness.ProjectID != "" && harness.Location != "" && harness.ClusterName != "" {
		envVars = append(envVars,
			...
			corev1.EnvVar{Name: "CREDENTIAL_PROXY_BOOTSTRAP_COMMAND", Value: `if command -v gcloud >/dev/null 2>&1 && [ -n "$GKE_PROJECT_ID" ] && [ -n "$GKE_CLUSTER_NAME" ] && [ -n "$GKE_LOCATION" ] && gcloud container clusters get-credentials "$GKE_CLUSTER_NAME" --location="$GKE_LOCATION" --project="$GKE_PROJECT_ID" >/dev/null 2>&1; then kubectl config set-context --current --namespace="$KUBE_DEFAULT_NAMESPACE" >/dev/null; else if [ -f /var/run/secrets/kubernetes.io/serviceaccount/ca.crt ]; then kubectl config set-cluster default --server="https://kubernetes.default.svc" --certificate-authority=/var/run/secrets/kubernetes.io/serviceaccount/ca.crt >/dev/null; else kubectl config set-cluster default --server="https://kubernetes.default.svc" --insecure-skip-tls-verify=true >/dev/null; fi && TOKEN_FILE=""; if [ -f /var/run/secrets/kubernetes.io/serviceaccount/token ]; then TOKEN_FILE="/var/run/secrets/kubernetes.io/serviceaccount/token"; elif [ -f /var/run/secrets/kubeagents/serviceaccount/token ]; then TOKEN_FILE="/var/run/secrets/kubeagents/serviceaccount/token"; fi && if [ -n "$TOKEN_FILE" ]; then kubectl config set-credentials in-cluster >/dev/null && kubectl config set users.in-cluster.tokenFile "$TOKEN_FILE" >/dev/null && kubectl config set-context default --cluster=default --user=in-cluster --namespace="$KUBE_DEFAULT_NAMESPACE" >/dev/null; else kubectl config set-context default --cluster=default --namespace="$KUBE_DEFAULT_NAMESPACE" >/dev/null; fi && kubectl config use-context default >/dev/null; fi`},
		)
	}
	if integration := agent.Spec.Integration; integration != nil {
		if gchat := integration.GoogleChat; gchat != nil && gchat.Enabled != nil && *gchat.Enabled {
