...
func buildCredentialProxyEnv(agent *agentv1alpha1.PlatformAgent) []corev1.EnvVar {
	...
	if harness := agent.Spec.Harness; harness != nil && harness.ProjectID != "" && harness.Location != "" && harness.ClusterName != "" {
		envVars = append(envVars,
			...
			corev1.EnvVar{Name: "CREDENTIAL_PROXY_BOOTSTRAP_COMMAND", Value: `if [ -f /var/run/secrets/kubernetes.io/serviceaccount/ca.crt ]; then kubectl config set-cluster default --server="https://kubernetes.default.svc" --certificate-authority=/var/run/secrets/kubernetes.io/serviceaccount/ca.crt >/dev/null; else kubectl config set-cluster default --server="https://kubernetes.default.svc" --insecure-skip-tls-verify=true >/dev/null; fi && TOKEN_FILE=""; if [ -f /var/run/secrets/kubeagents/serviceaccount/token ]; then TOKEN_FILE="/var/run/secrets/kubeagents/serviceaccount/token"; elif [ -f /var/run/secrets/kubernetes.io/serviceaccount/token ]; then TOKEN_FILE="/var/run/secrets/kubernetes.io/serviceaccount/token"; fi && if [ -n "$TOKEN_FILE" ]; then kubectl config set-credentials in-cluster --token="$(cat "$TOKEN_FILE")" >/dev/null && kubectl config set-context default --cluster=default --user=in-cluster --namespace="$KUBE_DEFAULT_NAMESPACE" >/dev/null; else kubectl config set-context default --cluster=default --namespace="$KUBE_DEFAULT_NAMESPACE" >/dev/null; fi && kubectl config use-context default >/dev/null`},
		)
	}
	if integration := agent.Spec.Integration; integration != nil {
		if gchat := integration.GoogleChat; gchat != nil && gchat.Enabled != nil && *gchat.Enabled {
