...
Both rosters are annotated on [Reference → Cron jobs](/kube-agents/reference/cron-jobs/), which also covers the four plumbing jobs on the Planning Agent's: the named-profile cron ticker described in [What fires the schedule](#what-fires-the-schedule), Cluster Agent reconciliation, and the two first-run onboarding steps. The Platform Agent's roster carries seven non-governance entries of its own: the `github-repo-watcher` poller, the k8s-event-watcher daily recap, the findings-queue morning nudge, the kanban workspace collector, `kanban-board-health`, which names cards stuck in `blocked` in chat, `chat-delivery-watch`, which reports a scheduled report that stopped reaching chat on a GitHub ledger issue and in Cloud Logging, and `feedback-prompt`, which asks the operator for feedback once.
